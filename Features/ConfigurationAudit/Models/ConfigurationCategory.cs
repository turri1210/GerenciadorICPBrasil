...
public class ConfigurationCategory : INotifyPropertyChanged
{
    ...
    public bool IsRemediationAvailable =>
        !IsLoading &&
        !IsWaiting &&
        Status == ConfigurationStatus.NonCompliant &&
        !string.Equals(Key, "sistema-operacional", StringComparison.OrdinalIgnoreCase);
    ...
    private static string GetDefaultMessageForStatus(ConfigurationStatus status) =>
        status switch
        {
            ...
            ConfigurationStatus.Pending => "Aguardando acao do usuario",
