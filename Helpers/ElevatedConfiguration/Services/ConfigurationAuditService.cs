...
public class ConfigurationAuditService : IConfigurationAuditService
{
    ...
    private static string? _lastWindowsUpdateError;
    ...
    // Ações externas são deliberadamente limitadas aos utilitários nativos necessários.
    // Nenhum comando, script ou caminho executável é recebido da interface do usuário.
    ...
    private static readonly AdvancedAuditSetting[] AdvancedAuditSettings =
    {
        ...
        new("Authorization Policy Change", AuditSettingState.SuccessAndFailure, AuditSubcategoryIds["Authorization Policy Change"], "auditoria de alteração de políticas de autorização"),
    ...
    private static readonly Dictionary<string, string[]> LocalizedAuditAliases = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["Authorization Policy Change"] = new[] { "Alteração na política de Autorização" },
        ...
        CancellationToken cancellationToken = default)
    {
        return await Task.Run(() =>
        {
            ...
            LogWriter.Write("Iniciando verificação: sistema-operacional");
            ...
            LogWriter.Write("Iniciando verificação: antivirus");
            ...
            LogWriter.Write("Iniciando verificação: windows-update");
            ...
            cancellationToken.ThrowIfCancellationRequested();
            stageProgress?.Report("proteção-tela");
            LogWriter.Write("Iniciando verificação: proteção-tela");
            var screenSaverAssessment = AnalyzeScreenSaver();
            LogWriter.Write($"Resultado proteção-tela: {screenSaverAssessment.Status}");
            Report("proteção-tela", screenSaverAssessment);

            cancellationToken.ThrowIfCancellationRequested();
            stageProgress?.Report("login-remoto");
            LogWriter.Write("Iniciando verificação: login-remoto");
            ...
            LogWriter.Write("Iniciando verificação: senha-forte");
            ...
            LogWriter.Write("Iniciando verificação: bloqueio-conta");
            ...
            LogWriter.Write("Iniciando verificação: auditoria-estacoes");
            ...
            LogWriter.Write("Iniciando verificação: contas-usuarios");
            ...
            LogWriter.Write("Iniciando verificação: visualizador-eventos");
            ...
            LogWriter.Write("Iniciando verificação: firewall");
            ...
            LogWriter.Write("Iniciando verificação: sincronismo-hora");
            ...
            LogWriter.Write("Iniciando verificação: criptografia");
            ...
            LogWriter.Write("Iniciando verificação: integridade");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.Run(() =>
        {
            ...
            LogWriter.Write($"Reavaliação direta ({categoryKey}): {result.Status}");
            return result;
        }, cancellationToken);
    }
        ...
        try
        {
            switch (categoryKey)
            {
                case "antivirus":
                    ...
                    break;
                case "windows-update":
                    await RemediateWindowsUpdateAsync(cancellationToken).ConfigureAwait(false);
                    break;
                case "protecao-tela":
    ...
    private static ConfigurationAssessmentResult EvaluateOperatingSystem()
    {
        try
        {
            ...
            if (key is null)
            {
                return new ConfigurationAssessmentResult(
                    ConfigurationStatus.Unknown,
                    "Não foi possível obter as informações do sistema operacional.");
            }
            ...
            if (BlockedEditionIds.Contains(editionId))
            {
                return new ConfigurationAssessmentResult(
                    ConfigurationStatus.NonCompliant,
                    "Instale a versão Windows 11 Pro para garantir os recursos de segurança necessários.");
            }
    ...
    private static WindowsUpdateAssessment AnalyzeWindowsUpdate()
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(_lastWindowsUpdateError))
            {
                return new WindowsUpdateAssessment(
                    new ConfigurationAssessmentResult(
                        ConfigurationStatus.NonCompliant,
                        $"Não foi possível configurar o Windows Update automaticamente: {_lastWindowsUpdateError}. Procure um tecnico para suporte."),
                    restartPending);
            }
    ...
    private static bool IsLatestUpdatesSettingEnabled()
    {
        ...
        var value1 = Convert.ToInt32(key.GetValue("IsContinuousInnovationOptedIn") ?? 0, CultureInfo.InvariantCulture);
        var value2 = Convert.ToInt32(key.GetValue("IsContinuousInnovationOn") ?? value1, CultureInfo.InvariantCulture);

        return value1 == 1 || value2 == 1;
    }

    private static bool IsMicrosoftUpdateSettingEnabled()
    {
        using var key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\WindowsUpdate\UX\Settings");
        if (key is null)
        {
        ...
        var allowMu = Convert.ToInt32(key.GetValue("AllowMUUpdateService") ?? 0, CultureInfo.InvariantCulture);
        return allowMu == 1;
    }
            ...
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (isBeta || IsExcludedWindowsUpdateTitle(title))
                        {
                            optionalCount++;
                            continue;
                        }
    ...
    private static bool IsExcludedWindowsUpdateTitle(string title)
    {
        var excludedTitleMarkers = new[]
        {
            "Preview",
            "Pré-visualização",
            "Versão prévia",
        ...
        return excludedTitleMarkers.Any(marker =>
            title.Contains(marker, StringComparison.OrdinalIgnoreCase));
    }

    private static async Task RemediateWindowsUpdateAsync(CancellationToken cancellationToken)
    {
        _lastWindowsUpdateError = null;

        try
        {
            ShowInformation("O Windows Update procurará, baixará e instalará automaticamente as atualizações recomendadas. O computador não será reiniciado automaticamente.");
            var result = await InstallWindowsUpdatesAsync(cancellationToken).ConfigureAwait(false);

            if (result.SelectedUpdates == 0)
            {
                var skippedMessage = result.SkippedUpdates > 0
                    ? $" {result.SkippedUpdates} atualização(ões) opcional(is), Preview, beta, de driver ou de recurso foram ignoradas."
                    : string.Empty;
                ShowInformation($"Nenhuma atualização recomendada está pendente.{skippedMessage}");
                return;
            }
            ...
            if (result.FailedUpdates > 0)
            {
                throw new InvalidOperationException(
                    $"{result.InstalledUpdates} de {result.SelectedUpdates} atualização(ões) foram instaladas; " +
                    $"{result.FailedUpdates} não puderam ser concluídas. {string.Join(" | ", result.Errors)}");
            }

            var restartMessage = result.RequiresRestart
                ? " Reinicie o computador para concluir a instalação."
                : string.Empty;
            ShowInformation($"{result.InstalledUpdates} atualização(ões) instalada(s) com sucesso.{restartMessage}");
        }
        catch (Exception ex)
        {
            _lastWindowsUpdateError = ex.Message;
            LogWriter.Write(ex, "Erro durante remediação do Windows Update");
            throw;
        }
    }

    private static Task<WindowsUpdateInstallSummary> InstallWindowsUpdatesAsync(CancellationToken cancellationToken)
    {
        return RunOnStaThreadAsync(() =>
        {
            ...
            object? availableUpdates = null;
            object? selectedUpdates = null;
            ...
            {
                var sessionType = Type.GetTypeFromProgID("Microsoft.Update.Session")
                    ?? throw new InvalidOperationException("A API oficial do Windows Update não está disponível.");
                session = Activator.CreateInstance(sessionType)
                    ?? throw new InvalidOperationException("Não foi possível iniciar uma sessão do Windows Update.");

                dynamic updateSession = session;
                updateSession.ClientApplicationID = "YeZ.GerenciadorICPBrasil";
                searcher = updateSession.CreateUpdateSearcher();
                dynamic updateSearcher = searcher;
                updateSearcher.Online = true;

                LogWriter.Write("Windows Update: procurando atualizações de software aplicáveis.");
                searchResult = updateSearcher.Search("IsInstalled=0 and Type='Software'");
                availableUpdates = ((dynamic)searchResult).Updates;

                var collectionType = Type.GetTypeFromProgID("Microsoft.Update.UpdateColl")
                    ?? throw new InvalidOperationException("A coleção da API do Windows Update não está disponível.");
                selectedUpdates = Activator.CreateInstance(collectionType)
                    ?? throw new InvalidOperationException("Não foi possível criar a lista de atualizações.");

                dynamic available = availableUpdates;
                dynamic selected = selectedUpdates;
                ...
                for (var index = 0; index < availableCount; index++)
                {
                    ...
                    try
                    {
                        updateObject = available.Item(index);
                        dynamic update = updateObject;
                        var title = Convert.ToString(update.Title, CultureInfo.CurrentCulture) ?? "Atualização sem título";

                        if (!ShouldAutomaticallyInstallWindowsUpdate(update, title))
                        {
                            skippedUpdates++;
                            LogWriter.Write($"Windows Update: atualização fora da política automática: {title}");
                            continue;
                        }
                        ...
                        selected.Add(update);
                        LogWriter.Write($"Windows Update: atualização selecionada: {title}");
                    }
                    finally
                    {
                ...
                if (selectedCount == 0)
                {
                    return new WindowsUpdateInstallSummary(0, 0, 0, skippedUpdates, false, Array.Empty<string>());
                }

                cancellationToken.ThrowIfCancellationRequested();
                downloader = updateSession.CreateUpdateDownloader();
                dynamic updateDownloader = downloader;
                updateDownloader.Updates = selected;
                LogWriter.Write($"Windows Update: baixando {selectedCount} atualização(ões).");
                downloadResult = updateDownloader.Download();

                var downloadResultCode = (int)((dynamic)downloadResult).ResultCode;
                if (downloadResultCode is 4 or 5)
                {
                    throw new InvalidOperationException($"O download das atualizações falhou (código WUA {downloadResultCode}).");
                }
                ...
                for (var index = 0; index < selectedCount; index++)
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    object? updateObject = null;
                    object? singleCollection = null;
                    object? installer = null;
                    object? installationResult = null;
                    object? updateResult = null;

                    try
                    {
                        updateObject = selected.Item(index);
                        dynamic update = updateObject;
                        var title = Convert.ToString(update.Title, CultureInfo.CurrentCulture) ?? "Atualização sem título";
                        ...
                        {
                            failedUpdates++;
                            errors.Add($"Download incompleto: {title}");
                            continue;
                        }

                        singleCollection = Activator.CreateInstance(collectionType)
                            ?? throw new InvalidOperationException("Não foi possível preparar uma atualização para instalação.");
                        ((dynamic)singleCollection).Add(update);

                        installer = updateSession.CreateUpdateInstaller();
                        dynamic updateInstaller = installer;
                        updateInstaller.ClientApplicationID = "YeZ.GerenciadorICPBrasil";
                        updateInstaller.AllowSourcePrompts = false;
                        updateInstaller.IsForced = false;
                        updateInstaller.Updates = singleCollection;

                        LogWriter.Write($"Windows Update: instalando: {title}");
                        installationResult = updateInstaller.Install();
                        dynamic aggregateResult = installationResult;
                        updateResult = aggregateResult.GetUpdateResult(0);
                        dynamic individualResult = updateResult;

                        var resultCode = (int)individualResult.ResultCode;
                        var hResult = (int)individualResult.HResult;
                        requiresRestart |= (bool)aggregateResult.RebootRequired || (bool)individualResult.RebootRequired;

                        if (resultCode == 2)
                        {
                            installedUpdates++;
                            LogWriter.Write($"Windows Update: instalação concluída: {title}");
                        }
                        else
                        {
                            failedUpdates++;
                            errors.Add($"{title} (resultado {resultCode}, HRESULT 0x{hResult:X8})");
                        }
                    }
                    finally
                    {
                        ReleaseComObject(updateResult);
                        ReleaseComObject(installationResult);
                        ReleaseComObject(installer);
                        ReleaseComObject(singleCollection);
                        ReleaseComObject(updateObject);
                    }
                }

                return new WindowsUpdateInstallSummary(
                    selectedCount,
                    installedUpdates,
                    failedUpdates,
                    skippedUpdates,
            ...
            finally
            {
                ...
                ReleaseComObject(selectedUpdates);
                ReleaseComObject(availableUpdates);
                ReleaseComObject(searchResult);
                ReleaseComObject(searcher);
                ReleaseComObject(session);
            }
    ...
    private static bool ShouldAutomaticallyInstallWindowsUpdate(dynamic update, string title)
    {
        bool isHidden = update.IsHidden;
        bool isBeta = update.IsBeta;
        bool autoSelected = update.AutoSelectOnWebSites;

        return SecurityPolicy.ShouldAutomaticallyInstallWindowsUpdate(
            isHidden,
            isBeta,
            autoSelected,
            title);
    ...
    private static async Task RemediateAntivirusAsync(CancellationToken cancellationToken)
    {
        ...
        {
            ShowInformation("Nenhum antivirus ativo foi identificado. O Microsoft Defender sera habilitado e atualizado.");
        ...
        {
            ShowInformation("O antivirus esta desatualizado. Buscando novas assinaturas...");
    ...
    private static async Task EnableWindowsDefenderAsync(CancellationToken cancellationToken)
    {
        ...
        ShowInformation("Abra Segurança do Windows para revisar e ativar o Microsoft Defender. O aplicativo não altera preferências do antivírus por script.");
    ...
    private static Task UpdateWindowsDefenderSignaturesAsync(CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        OpenWindowsSettings("windowsdefender:");
        ShowInformation("Revise a atualização das definições do Microsoft Defender em Segurança do Windows.");
    ...
    private static ConfigurationAssessmentResult AnalyzeScreenSaver()
    {
        try
        {
            ...
            {
                issues.Add("Ative a opção de exibir a tela de logon ao retornar.");
            }
            ...
            var message = "Ajuste a proteção de tela: " + string.Join(" ", issues);
            return new ConfigurationAssessmentResult(ConfigurationStatus.NonCompliant, message);
        }
        catch (Exception ex)
        {
            LogWriter.Write(ex, "Erro ao verificar configuração de proteção de tela");
            return new ConfigurationAssessmentResult(ConfigurationStatus.Unknown, $"Não foi possível verificar a proteção de tela: {ex.Message}");
        }
    }
    ...
    private static ConfigurationAssessmentResult AnalyzeRemoteAssistance()
    {
        try
        {
            ...
            {
                issues.Add("Desative a opção 'Permitir conexões de Assistencia Remota para este computador'.");
            }
    ...
    private static async Task RemediateRemoteAssistanceAsync(CancellationToken cancellationToken)
    {
        ...
        LogWriter.Write("Assistencia Remota desativada.");
    }
    ...
    private static ConfigurationAssessmentResult AnalyzeAccountLockoutPolicy()
    {
        try
        {
            ...
            {
                issues.Add("Defina a duração do bloqueio de conta para 10080 minutos.");
            }
        ...
        try
        {
            ...
            foreach (var key in BaseAuditPolicyKeys)
            {
                    ...
                    value != expectedValue)
                {
                    ...
                    issues.Add($"Defina '{friendly}' como '{expectation}' nas políticas básicas.");
                }
            }
    ...
    private static ConfigurationAssessmentResult AnalyzeApplicationEventLog()
    {
        try
        {
            ...
            return new ConfigurationAssessmentResult(
                ConfigurationStatus.Compliant,
                "Log de Aplicativo configurado com tamanho máximo de 4194240 KB.");
        }
    ...
    private static async Task RemediateBitLockerConfigurationAsync(CancellationToken cancellationToken)
    {
        ...
        ShowInformation("Clique em 'Ligar BitLocker', escolha 'Inserir uma senha', salve a senha e a chave de recuperação. Selecione 'Criptografar a unidade inteira', 'Modo compatível', marque 'Executar verificação do sistema BitLocker' e conclua o assistente. Reinicie o computador ao final.");
    ...
    private static async Task RemediateIntegrityConfigurationAsync(CancellationToken cancellationToken)
    {
        ...
        ShowInformation("Auditoria configurada. Utilize o Visualizador de Eventos para monitorar gravações nas pastas do BiometricLocalServicePlataform.");
    }
    ...
    private static async Task RemediateAccountLockoutPolicyAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogWriter.Write("política de bloqueio de conta aplicada (duração, limite, redefinição).");
        }
        finally
        {
            try
    ...
    private static UserAccountsPlan? ShowUserAccountsPlanDialog(IReadOnlyList<LocalUserAccountPlanItem> existingAccounts)
    {
        ...
        {
            // Neste helper sem janela, o primeiro Window criado torna-se MainWindow.
            // Definir MainWindow como Owner no inicializador faria o diálogo ser dono
            // de si mesmo e provocaria "Não é possível definir a propriedade Owner
            // como ela mesma" antes de o plano ser exibido.
            var owner = Application.Current.MainWindow;
            var dialog = new UserAccountsPlanDialog(existingAccounts);
    ...
    private static Dictionary<string, AuditSettingState> ParseAuditPolTable(string output)
    {
        ...
        foreach (var rawLine in lines)
        {
            ...
            if (normalized.Contains("categoria/subcategoria") ||
                ...
                normalized.Contains("configuração") ||
    ...
    {
        if (uri is not ("ms-settings:windowsupdate" or "windowsdefender:"))
        {
    ...
    private static void ShowInformation(string message)
    {
        if (Application.Current?.Dispatcher is { } dispatcher)
        {
            dispatcher.Invoke(() =>
            {
                MessageBox.Show(message, "ConfigAuditoria", MessageBoxButton.OK, MessageBoxImage.Information);
            });
        }
    }
    ...
    private static void ShowError(string message)
    {
        if (Application.Current?.Dispatcher is { } dispatcher)
        {
            dispatcher.Invoke(() =>
            {
                MessageBox.Show(message, "ConfigAuditoria", MessageBoxButton.OK, MessageBoxImage.Error);
            });
        }
    }
    ...
    private sealed record WindowsUpdateInstallSummary(
        int SelectedUpdates,
